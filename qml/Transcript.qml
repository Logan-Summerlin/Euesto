...
Rectangle {
    ...
    function updateTailState() {
        followingTail = maximumY() - viewport.contentY <= 24
    }

    function followTail() {
        if (!followingTail || userScrolling)
            return
        movingToTail = true
        viewport.contentY = maximumY()
        Qt.callLater(function() { movingToTail = false })
    }

    function activityExpanded(key, defaultValue) {
        let value = activityOverrides[String(key || "")]
        return value === undefined ? defaultValue : value
    }

    function setActivityExpanded(key, expanded) {
        let next = {}
        for (let name in activityOverrides)
            next[name] = activityOverrides[name]
        next[String(key || "")] = expanded
    ...
    Flickable {
        ...
        ScrollBar.vertical: ScrollBar {
            ...
            contentItem: Rectangle {
                ...
                opacity: transcriptScrollBar.pressed || transcriptScrollBar.hovered ? 0.95 : 0.65
            }
            onPressedChanged: {
                if (pressed) {
                    ...
                    root.updateTailState()
                }
            }
        }

        onMovementStarted: {
            if (!root.movingToTail) {
                root.userScrolling = true
                root.followingTail = false
            }
        }
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    Column {
                        ...
                        Button {
                            id: activityButton
                            property bool expanded: root.activityExpanded(
                                card.value.key, card.value.activityExpanded === true
                            )
                            width: parent.width
                            height: visible ? implicitHeight : 0
                            visible: (card.value.activity || []).length > 0
                            flat: true
                            text: (expanded ? "▾ " : "▸ ")
                                + (card.value.activitySummary || "Tool calls")
                            palette.buttonText: root.mutedColor
                            onClicked: root.setActivityExpanded(card.value.key, !expanded)
                        }
                        ...
                        Loader {
                            ...
                            sourceComponent: Column {
                                ...
                                spacing: 4
                                Repeater {
                                    model: card.value.activity || []
                                    delegate: Rectangle {
                                        id: activityEvent
                                        required property var modelData
                                        readonly property var value: modelData || ({})
                                        width: activityLoader.width
                                        height: Math.ceil(eventTitle.implicitHeight + 14)
                                        radius: 7
                                        color: activityEvent.value.attention === true
                                            ? Qt.rgba(0.75, 0.25, 0.2, 0.12) : "transparent"
                                        border.color: root.borderColor
                                        Label {
                                            id: eventTitle
                                            anchors.fill: parent
                        ...
                        TextEdit {
                            id: messageBody
                            objectName: "transcriptMessageBody"
                            width: parent.width
    ...
    Dialog {
        ...
        modal: true
        anchors.centerIn: parent
        width: Math.min(600, root.width - 60)
        standardButtons: Dialog.Ok | Dialog.Cancel
        function openFor(id, value) {
            messageId = id
            editor.text = value
            open()
        }
