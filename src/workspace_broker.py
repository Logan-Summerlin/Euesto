...
class WorkspaceBroker:
    ...
    def publish(self, manifest: PublishManifest, approved_paths: set[str]) -> PublishResult:
        ...
        metadata: dict[str, dict[str, str | bool | int | None]] = {}
        completed: list[str] = []
        touched: list[str] = []
        try:
            for operation in manifest.operations:
                ...
                if operation.operation == "delete":
                    ...
                    _atomic_write(target, operation.payload(), operation.staged_mode)
    ...
    def _restore_touched(self, checkpoint: Path, metadata: dict[str, dict[str, str | bool | int | None]], touched: list[str]) -> tuple[list[str], list[str]]:
        ...
        for relative in reversed(touched):
            ...
            try:
                ...
                if item.get("existed"):
                    _atomic_write(target, (checkpoint / "files" / relative).read_bytes(), _base_mode(item))
    ...
    def undo(self, checkpoint_id: str) -> PublishResult:
        ...
        for relative, item in metadata.items():
            ...
            if item["existed"]:
                _atomic_write(target, recovery.read_bytes(), _base_mode(item))
            elif target.exists():
                target.unlink()
            completed.append(relative)
        return PublishResult(checkpoint_id, tuple(completed))
...
def _atomic_write(target: Path, content: bytes, mode: int | None = None, *, prefix: str = ".local-chat-") -> None:
...
def _base_mode(item: dict) -> int | None:
    return int(item["base_mode"]) if item.get("base_mode") is not None else None
...
class PublicationLedger:
    ...
    def record(self, manifest: PublishManifest, status: str, *, checkpoint_id: str | None = None, error: str | None = None) -> dict:
        ...
        _atomic_write(self._path(manifest.publication_id), json.dumps(record, indent=2, sort_keys=True).encode("utf-8"), prefix=".ledger-")
        return record
