...
class ChatWorker(QThread):
    ...
    def __init__(
        ...
        server_tools: ServerToolOptions,
    ...
    def run(self) -> None:
        ...
        try:
            for event in self.client.stream_chat(
                ...
                if run_id and run_id != started_run_id:
                    started_run_id = str(run_id)
...
class AgentWorker(QThread):
    ...
    def for_run(
        ...
        *,
        ...
        return cls(
            client,
            lambda stop_event: client.stream_agent(
                api_key=api_key,
                model=model,
                messages=messages,
                mode=mode,
            ...
            auto_approve=approval_policy == "auto",
        )
    ...
    def for_resume(cls, client: GatewayClient, run_id: str, api_key: str) -> AgentWorker:
        return cls(
            client,
            lambda stop_event: client.resume_agent(
                run_id, api_key=api_key, stop_event=stop_event
            ),
            failure_context="Agent resume",
        )
        ...
        try:
            for event in self._stream(self.stop_event):
                ...
                if event.type == "model.delta" and event.payload.get("text"):
                    ...
                    continue
                if event.type in {"usage.updated", "run.completed"}:
                    usage.update(event.payload)
                    continue
                # The gateway journal owns the exact event stream. High-volume model
                # deltas and potentially enormous tool output never cross into QML.
                if event.type in {"tool.output", "tool.completed"}:
                    continue
...
class StagingDiscardWorker(QThread):
    ...
    def __init__(self, client: GatewayClient, workspace_id: str):
        super().__init__()
        self.client = client
    ...
    def run(self) -> None:
        try:
            self.complete.emit(self.client.discard_staging(self.workspace_id))
        except (GatewayError, KeyError, TypeError, ValueError) as exc:
            self.failed.emit(str(exc))
...
class StagingInspectWorker(QThread):
    ...
    def run(self) -> None:
        try:
            self.complete.emit(self.client.inspect_staging(self.workspace_id))
        except (GatewayError, KeyError, TypeError, ValueError) as exc:
            self.failed.emit(str(exc))
...
class PublicationWorker(QThread):
    ...
    def __init__(
        ...
        recovery_root: Path,
        *,
        reseed_client: GatewayClient | None = None,
    ):
        super().__init__()
    ...
    def run(self) -> None:
        ...
        try:
            ...
            published = broker.publish(
                self.manifest, {item.path for item in self.manifest.operations}
            )
