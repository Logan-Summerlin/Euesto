...
def test_edit_reports_actual_changed_lines(tmp_path: Path) -> None:
    ...
    assert result.data["diff"]["changed_lines"] == 1
...
def test_bounded_diff_labels_truncation(tmp_path: Path) -> None:
    old = "\n".join(f"old-{i}" for i in range(500)); new = "\n".join(f"new-{i}" for i in range(500))
...
def test_status_reports_staged_changes_and_publication_review(tmp_path: Path) -> None:
    ...
    service = ExecutorService(make_config(source, tmp_path / "work")); (service.config.work_root / "new.py").write_text("print('new')\n", encoding="utf-8")
...
def test_chmod_is_preserved_in_staging_and_publication(tmp_path: Path) -> None:
    ...
    service = ExecutorService(config); staged = service.config.work_root / "run.sh"
    ...
    manifest = service.manifest("run", "approval"); operation = manifest.operations[0]
