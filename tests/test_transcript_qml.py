...
ApplicationWindow {
    ...
    # create() gives the root object JavaScript ownership. In a headless test,
    # deferred QML garbage collection can therefore destroy the C++ window while
    # Python still holds its shiboken wrapper. Keep both the component and root
    # alive for the complete test and make Python/C++ ownership explicit.
...
def _message_bodies(transcript: QObject, count: int) -> list[QObject]:
    ...
    return [_transcript_repeater(transcript).itemAt(index).findChild(QObject, "transcriptMessageBody") for index in range(count)]


def test_transcript_scroll_anchor_survives_a_tail_update(qapp: QApplication) -> None:
    backend = FakeBackend(); rows = [_message(index, (1, 4, 16, 3)[index % 4]) for index in range(1, 41)]; backend.model.replace(rows, reset=True)
    engine, component, window = _create_transcript_window(backend)
    try:
        QTest.qWait(100); transcript = window.findChild(QObject, "transcriptRoot"); viewport = window.findChild(QObject, "transcriptViewport"); assert transcript is not None and viewport is not None
        initial_height = float(viewport.property("contentHeight")); assert initial_height > float(viewport.property("height")); transcript.setProperty("followingTail", False); viewport.setProperty("contentY", initial_height / 2); QTest.qWait(20); anchored_y = float(viewport.property("contentY"))
        ...
        stable_height = float(viewport.property("contentHeight")); viewport.setProperty("contentY", 0); QTest.qWait(20); viewport.setProperty("contentY", stable_height - float(viewport.property("height"))); QTest.qWait(20); assert float(viewport.property("contentHeight")) == pytest.approx(stable_height, abs=0.5)
        transcript.setProperty("followingTail", True); rows_at_tail = changed + [_message(41, 24)]; backend.model.replace(rows_at_tail); backend.transcriptChanged.emit(); QTest.qWait(100)
...
def test_transcript_renders_long_content_after_an_existing_row_update(qapp: QApplication) -> None:
    ...
    try:
        QTest.qWait(100); transcript = window.findChild(QObject, "transcriptRoot"); assert transcript is not None; bodies = _message_bodies(transcript, 2); before = float(bodies[-1].property("contentHeight"))
        updated = list(rows); updated[-1] = _message(2, 180); backend.model.replace(updated); bodies = _message_bodies(transcript, 2); after = bodies[-1]; content_height = float(after.property("contentHeight")); assert content_height > before; assert float(after.property("height")) == pytest.approx(content_height, abs=1.0); assert str(after.property("text")).startswith("<p>line 0")
    finally:
        _destroy_transcript_window(qapp, engine, component, window)


def test_transcript_keeps_all_rows_after_repeated_scroll_and_height_updates(qapp: QApplication) -> None:
    backend = FakeBackend(); rows = [_message(index, (2, 5, 18, 3)[index % 4]) for index in range(1, 61)]; backend.model.replace(rows, reset=True); engine, component, window = _create_transcript_window(backend)
    try:
        QTest.qWait(120); transcript = window.findChild(QObject, "transcriptRoot"); viewport = window.findChild(QObject, "transcriptViewport"); assert transcript is not None and viewport is not None; initial_height = float(viewport.property("contentHeight")); viewport_height = float(viewport.property("height")); assert initial_height > viewport_height; _message_bodies(transcript, len(rows)); transcript.setProperty("followingTail", False)
        ...
        changed = list(rows); changed[10] = _message(11, 45); changed[48] = _message(49, 32); backend.model.replace(changed); _message_bodies(transcript, len(changed))
        for fraction in (1.0, 0.0, 0.5, 0.17, 0.83, 0.0, 1.0):
            maximum = max(0.0, float(viewport.property("contentHeight")) - viewport_height); viewport.setProperty("contentY", maximum * fraction); QTest.qWait(15)
        bodies = _message_bodies(transcript, len(changed)); assert all(body is not None and float(body.property("height")) > 0 for body in bodies); assert float(viewport.property("contentHeight")) > initial_height
    finally:
        _destroy_transcript_window(qapp, engine, component, window)
