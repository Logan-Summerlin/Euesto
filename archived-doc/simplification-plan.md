...
- **Staged rewrites drop file modes.** `write`, `edit`, and `apply_patch` replace a file through a `mkstemp` temporary file, so the result has mode `0600`. Every rewrite of an existing `0644` file therefore reports a permission change in `status` and `workspace_status`. The staged mode is also applied on the host at publication, which matters on POSIX hosts. It changes publication behavior, so it was reported instead of folded into this pass. **Fixed after the pass:** a replaced file now keeps its mode and a new file gets the umask default (`tests/unit/executor/test_file_modes.py`).
