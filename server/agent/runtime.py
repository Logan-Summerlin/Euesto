...
class AgentRuntime:
    ...
    async def run(self, run_id: str, request: AgentRunRequest, api_key: str, *, initial_messages=None, visible_messages=None, budget_state=None, resumed=False) -> None:
        ...
        try:
            ...
            await self.append(run_id, "context.inspected", {"estimated_tokens": estimate_message_tokens(messages), "limit_tokens": request.context_limit_tokens, "message_count": len(messages), "session_id": request.session_id})
            self._save_snapshot(run_id, request, messages, visible, budget, True)
            while True:
                if self.pause_requested(run_id):
                    self._save_snapshot(run_id, request, messages, visible, budget, True)
                    await self.append(run_id, "run.paused", {"reason": "user.requested", "resumable": True})
                    return
                ...
                if not turn.tool_calls:
                    ...
                    await self.append(run_id, "run.completed", {"iterations": budget.iterations, **budget.usage()})
                    return
            ...
            await self.append(run_id, "run.failed", {"code": exc.code, "message": str(exc), "retryable": exc.retryable})
        except Exception as exc:
            await self.append(run_id, "run.failed", {"code": "agent.failed", "message": str(exc)[:2000], "retryable": False})
        finally:
...
def _render_executor_context(status: dict[str, Any], mode: str, approval_policy: str = "prompt") -> str:
    ...
    if mode == "agent":
        lines.append("- Tools write an ephemeral staged copy; host publication requires review unless session Auto is active.")
    ...
    if isinstance(limits, dict):
        lines.append(f"- Hard limits: read {limits.get('max_read_bytes', 0)} bytes, output {limits.get('max_output_bytes', 0)} bytes, command {limits.get('max_command_seconds', 0)} seconds.")
    return "\n".join(lines)[:1600]
