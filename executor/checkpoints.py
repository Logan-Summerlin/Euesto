...
def create_checkpoint(
    ...
    *,
    ...
    """Create a bounded, content-addressed snapshot of staging."""
    files = visible_files(work_root)
    total = sum(size for _digest, size in files.values())
    ...
    directory.mkdir()
    try:
        for relative, (digest, _size) in files.items():
        ...
        manifest = {
            ...
            "files": {
                relative: {"sha256": digest, "size_bytes": size}
...
def restore_checkpoint(
    ...
    *,
    ...
    """Preview or restore a checkpoint without leaving a partial prepared set."""
    manifest = _load_manifest(work_root, checkpoint_id)
    ...
    total = sum(size for _digest, size in expected.values())
    ...
    changed = [
        ...
        if current.get(relative, (None, None))[0] != expected.get(relative, (None, None))[0]
    ]
    if preview:
        return {
            "checkpoint_id": checkpoint_id,
    ...
    # Read and verify every replacement before changing the staging tree.
    prepared: dict[str, bytes] = {}
    objects = work_root / ".local-chat-checkpoints" / "objects"
    for relative in changed:
        if relative not in expected:
            continue
        digest, _size = expected[relative]
        object_path = objects / digest
        if not object_path.is_file() or _sha256(object_path) != digest:
            raise CheckpointError("Checkpoint content is missing or corrupt.")
        prepared[relative] = object_path.read_bytes()
    ...
    for relative in changed:
        ...
        _atomic_bytes(target, prepared[relative])
    return {
        "checkpoint_id": checkpoint_id,
        "preview": False,
...
def discard_staging(config: object) -> object:
    """Delete only the private staging volume and reseed it from the source."""
...
def _prune(root: Path, current_id: str, max_count: int, max_storage_bytes: int) -> None:
    ...
    object_files = sorted(
        (path for path in objects.iterdir() if path.is_file()),
        key=lambda path: path.stat().st_mtime,
    ...
    for path in object_files:
        if total <= max_storage_bytes or path.name in referenced:
            continue
